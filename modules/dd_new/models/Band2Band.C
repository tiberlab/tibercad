...
Band2Band::calculate_rate_and_derivatives(std::vector<double>& R, std::vector<std::vector<double>>& dPotentials)
{
  ...
  else
  {
    ...
    libMesh::RealGradient gradf1;
    libMesh::RealGradient gradf2;
    dd.get_grad_fermi(id1, gradf1);
    dd.get_grad_fermi(id2, gradf2);

    double nonloc1 = (E02 - E01) / (F*F) * fabs(dd.get_electric_field() * gradf2);
    double nonloc2 = (E02 - E01) / (F*F) * fabs(dd.get_electric_field() * gradf1);

    double exponential1 = exp((Ef2 - Ef1 - nonloc1) * beta);
    double exponential2 = exp((Ef1 - Ef2 - nonloc2) * beta);
