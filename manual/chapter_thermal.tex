...
The steady state heat equation with the continuity equation read as
...
Negleting particle effects, the thermal conductivity is only due to the lattice contribution. The lattice thermal conductivity is readen from the database.
...
The physical model \textbf{heat\_source} allows to include a specific source identified by its \textbf{model} name.
...
With the notation \textit{drift\_diffusion\_simulation = dd\_simul\_name} we indicate the drift diffusion simulation name.

\subsection{Boundary conditions}


The boundary conditions can fix the temperature at a specific contact. This Dirichlet like condition can be interpreted as a heat reservoir and is identified as \textit{type = heat\_reservoir}. Below is reported an example.
...
The available quantities are \textit{T} (nodal-based variable) and \textit{HeatSource} (element-based variable).
The former allows to plot the temperature and the latter refers to all sources considerated in the simulation. In the table we report all sources associated to drift diffusion simulations.
...
$qR\left(\phi_p - \phi_n + T \left( P_p - P_n \right) \right)$ & \textit{Recombination effect} &  Rec\\
