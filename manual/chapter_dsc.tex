...
illumination. All mobilities, except electron mobility, are considered constant. For electron mobility
a multitrapping model has been implemented [REF], the TiberCAD user can chose if 
compute the electrical behavior of the cell including or not including traps (with an exponential distribution).

The Poisson equation to handle the internal
potential drop:
...
\begin{equation}\label{ricombinazione2}
   k_e = k_0 \left ( \right) ....
\end{equation}
