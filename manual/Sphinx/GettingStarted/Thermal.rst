...
The surface resistance can be added with the keyword boundary ``surface_resistance``.
Surface_resistance is  given  by  the  keyword ``r_surf``  in  :math:`m^{2} W/K`. 
...
The heat source is  given  by  a  constant power density :math:`H` expressed  in  :math:`W/m^{3}`.   
...
Heat generated by the Joule's effect is included with the  model type ``joule``. 
