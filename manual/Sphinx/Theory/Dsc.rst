...
:math:`E_{red}` is the redox potential. The redox potential can be evaluated using a Nernst approximation:
...
the boundary of a region where :math:`TiO_2` is present, on the contrary the cathode must be
...
in order to have no current during the light intensity sweep. 
In case a simulation under dark conditions
...
where :math:`\alpha` is the absorption coefficient (in :math:`\mu m^{-1}` ) of the chosen Dye, 
 :math:`\Phi(\lambda)` the intensity of
...
to change the spectrum profile F with another illumination source (for example if it is
assumed that the cell is under light concentration). The file used by default for :math:`\Phi` is the
...
Finally, the voltage sweep compute the I-V characteristics under illumination. 
In case of dark simulation (application of an external bias without illumination) the first and second sweep are not
...
The intensity of illumination can be changed sweeping the value of x different to 1 
(1 = Sun of power).
