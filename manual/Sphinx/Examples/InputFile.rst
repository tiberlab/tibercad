...
Each block may contain zero or any number of **parameter assignements** in the
form:
...
**Device** section includes the geometrical desription of the device to be simulated; an
...
The **Region** blocks contain the description of the device in continuous media ap 
proach; the **Cluster** blocks define each a group of regions (mesh regions) even with
...
ture to be be simulated in **TiberCAD** . In the rest of the input file, the physical regions
associated to the **Modules** will be indicated by means of the **TiberCAD Region** and
 **Cluster** names.
...
indicates that the values are applied to the variable Vg, which is a quantity defined in a
 **Contact** definition
...
sweep and that will be shown in the output file sweep modelname sweepvariable.dat,
eg. sweep driftdifiusion Vb.dat for a sweep of current calculation on the variable Vb
...
the **selfconsistent** keyword like a simulation name, in any **solve** assignement, e.g. 
