...
The Physics section of the input file is used to allow a custom bond stretching parameter ``\alpha`` and a custom bond bending parameter ``\beta`` through the definition of blocks defined with the keyword ``keating user``. An example of custom parameters is ::
