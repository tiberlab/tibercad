...
close to a specific number, referred to as the *spectral_shift*
  ...
  ``spectral_shift = double`` : 
     the closest eigenvalues to this value (eV) are found. 
...
The available options are:
  ...
  ``analytic`` :  bool
     { true | false } If true then the density is calculated analytically,
...
The parameters are the following:
  ...
  ``k_space_dimension`` : 
     **1** for 2D simulations, **2** for 1D simulations. 
     k-space basis is **true** if the k-space is defined by means of k-vectors; 
