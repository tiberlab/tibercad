    ...
    double& recomb_h)
{
  ...
  if (this->coupling() == "HG")
  {
    n = dd.get_electron_density_guest();
    p = dd.get_hole_density_guest();

    double Efn = dd.get_electron_electro_chemical_potential_guest();
    double Efp = dd.get_hole_electro_chemical_potential_guest();
    double kT = dd.get_lattice_temperature();

    double arg = (Efp - Efn) / kT;
    double fac = 1 - exp(-arg);
    recomb_e = recomb_h = C_ * n * p * fac;
  }
}
