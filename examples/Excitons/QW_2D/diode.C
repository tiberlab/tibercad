...
int main (int argc, char** argv)
{
  libMesh::init(argc, argv);
  {
    ...
    double in_content = input_file("in_content", in_content);
    ...
    phys_reg_ID[0] = 1; // n_bulk
    phys_reg_ID[1] = 2; // QW
    phys_reg_ID[2] = 3; // n_top
    phys_reg_ID[3] = 4; // p
    ...
    Macrostrain::strain_param n_strain;
    Macrostrain::strain_param qw_strain;
    Macrostrain::strain_param ntop_strain;
    Macrostrain::strain_param p_strain;
    Piezoelectricity n_piezo;
    Piezoelectricity qw_piezo;
    Piezoelectricity ntop_piezo;
    Piezoelectricity p_piezo;

    std::map<unsigned int, Macrostrain::strain_param*> strain_params;
    strain_params[1] = &n_strain;
    strain_params[2] = &qw_strain;
    strain_params[3] = &ntop_strain;
    strain_params[4] = &p_strain;

    std::map<unsigned int, Piezoelectricity*> piezodata;
    piezodata[1] = &n_piezo;
    piezodata[2] = &qw_piezo;
    piezodata[3] = &ntop_piezo;
    piezodata[4] = &p_piezo;

    n_strain.crystal = cryst;
    qw_strain.crystal = cryst;
    ntop_strain.crystal = cryst;
    p_strain.crystal = cryst;
    ...
    // GaN
    n_strain.C_tensor.set_moduli(390.0, 145.0, 106.0, 398.0, 105.0);
    ntop_strain.C_tensor.set_moduli(390.0, 145.0, 106.0, 398.0, 105.0);
    p_strain.C_tensor.set_moduli(390.0, 145.0, 106.0, 398.0, 105.0);
    //
    n_piezo.set_moduli(1.27, -0.35, -0.3); // C/m^2
    ntop_piezo.set_moduli(1.27, -0.35, -0.3); // C/m^2
    p_piezo.set_moduli(1.27, -0.35, -0.3); // C/m^2
    //
    n_piezo.set_pyro_module(-0.034);
    ntop_piezo.set_pyro_module(-0.034);
    p_piezo.set_pyro_module(-0.034);
    //
    n_strain.crystal.set_lat_const(0.3189, 0.5185);
    ntop_strain.crystal.set_lat_const(0.3189, 0.5185);
    p_strain.crystal.set_lat_const(0.3189, 0.5185);
    n_strain.crystal.calculate_lat_consts();
    ntop_strain.crystal.calculate_lat_consts();
    p_strain.crystal.calculate_lat_consts();
    n_strain.crystal.calculate_rot_matrix(x_dir, y_dir); 
    ntop_strain.crystal.calculate_rot_matrix(x_dir, y_dir); 
    p_strain.crystal.calculate_rot_matrix(x_dir, y_dir); 
    n_strain.C_tensor.rotate_to_calc_system(n_strain.crystal.RotMatrix);
    ntop_strain.C_tensor.rotate_to_calc_system(ntop_strain.crystal.RotMatrix);
    p_strain.C_tensor.rotate_to_calc_system(ntop_strain.crystal.RotMatrix);

    // InGaN
    // first value: InN, second value: GaN
    qw_strain.C_tensor.set_moduli(
    ...
    qw_piezo.set_pyro_module(
        alloy(-0.042, -0.034, in_content, -0.037));

    qw_strain.crystal.set_lat_const(
        alloy(0.3545, 0.3189, in_content),
        alloy(0.5703, 0.5185, in_content));
    qw_strain.crystal.calculate_lat_consts();
    qw_strain.crystal.calculate_rot_matrix(x_dir, y_dir);
    qw_strain.C_tensor.rotate_to_calc_system(qw_strain.crystal.RotMatrix);
    ...
    SemiconductorModel ntop;    
    ntop.set_data_file(material);
    ntop.read_database(d);
    
    ntop.add_dopant(new Dopant(n_doping, 0.025, 2, Dopant::N_TYPE));
    ntop.set_mobilities(mu_e, mu_h);

    opts["tau_n"] = tau_n;
    opts["tau_p"] = tau_n;
    opts["C"] = C_direct;
    rm = RecombinationModelInterface::create("SRH", opts);
    ntop.add_recombination_model(rm);
    rm = RecombinationModelInterface::create("direct", opts);
    ntop.add_recombination_model(rm);
    opts["C"] = X_gen;
    rm = RecombinationModelInterface::create("exciton_generation", opts);
      ...
      for ( ; it != end; ++it)
      {
        ...
        switch (id)
        {
          case 2:
            ...
            break;
          case 3:
            element_data.set_data(elem, &ntop);
            break;
          case 4:
    ...
    dd.set_simulation_voltage("anode", 0.0);
    {
      ...
      GMVIO(ex.get_mesh()).write_nodal_data("output/excitons_eq.gmv",
          densities, names);
    }
    ...
    cout << "\nBegin sweep...\n" << flush;
    params.solver_params.pc_type = PCCOMPOSITE;
    //params.solver_params.pc_type = PCJACOBI;
    params.coupling = FULLYCOUPLED;
