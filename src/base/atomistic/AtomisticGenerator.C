...
const double AtomisticGenerator::scale = 10.0;
...
//! Fast bond map generation (Order N), suitable for both
   unsigned int** AtomisticGenerator::fast_bond_map(const std::vector<Atom> &basis,
		   Tensor1& edge_min, Tensor1& edge_max, Tensor2Gen& period)
   {
	   ...
	   double minimum_spacing = 10.0;

	   std::vector<std::vector<std::vector<std::vector<unsigned int> > > >  grid_cell;
       ...
       //Define the grid, edge_max must be enlarged a little just to avoid strict inclusion problems
	   //-----------------------------------
	   ...
	   n_x = (floor(lenght_x / minimum_spacing)); x_spacing = lenght_x / n_x;
	   n_y = (floor(lenght_y / minimum_spacing)); y_spacing = lenght_y / n_y;
	   n_z = (floor(lenght_z / minimum_spacing)); z_spacing = lenght_z / n_z;
//-----------------------------------


	   unsigned int n_atom = basis.size();

	   //Resize array for x, y, z
//-------------------------------------
	   grid_cell.resize(n_x);
for (x = 0; x < n_x; x++)
{
	grid_cell[x].resize(n_y);
	for (y = 0; y < n_y; y++)
	{
		grid_cell[x][y].resize(n_z);
	}
}

//Assign atoms (by index) in grid_cell array
//------------------------------------
for (i = 0; i < n_atom; i++)
	   {
		   x = ( (floor(basis[i].get_position()(1) - edge_min(1)) / x_spacing ));
		   y = ( (floor(basis[i].get_position()(2) - edge_min(2)) / y_spacing ));
		   z = ( (floor(basis[i].get_position()(3) - edge_min(3)) / z_spacing ));
		   grid_cell[x][y][z].push_back(i);
	   }
//--------------------------------------


//Cycle upon outer cells (periodical bonds can exist only in these loops)
//----------------------------------------
//x outer cells
