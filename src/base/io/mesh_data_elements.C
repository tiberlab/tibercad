...
// //------------------------------------------------------
// //  MeshData_elements functions
// MeshData_elements::MeshData_elements(const MeshBase& m) :
//   _mesh               (m),
//   _data_descriptor    (""),
...
//   Constructor  derived  from  base  class (see .h )
//   Destructor  MUST  be  re-implemented  in  this  derived  class


MeshData_elements::~MeshData_elements()
{
  clear();
}
...
// void MeshData_elements::activate (const std::string& descriptor)
// {
// #ifdef DEBUG
//   if (_compatibility_mode)
//       std::cerr << "WARNING: MeshData_elements was in compatibility mode, now being activated."
// 		<< std::endl;
// #endif
...
// void MeshData_elements::enable_compatibility_mode (const std::string& descriptor)
// {
//   if (!_active)
//     {
...
// void  MeshData_elements::slim (const bool node_id_map,
...
//     {
//       std::cerr << "WARNING: No need for MeshData_elements::slim() in compatibility mode." << std::endl;
//     }
// #endif
// }
...
// void MeshData_elements::translate (const MeshBase& out_mesh,
// 			  std::vector<Number>& values,
// 			  std::vector<std::string>& names) const
// {
//   assert (_active || _compatibility_mode);

//   START_LOG("translate()", "MeshData_elements");
...
//   STOP_LOG("translate()", "MeshData_elements");
// }


// *********************************************
//  NEW :   translate  for  elements  data
//  *****************************************
...
void MeshData_elements::translate_elem_data (const MeshBase& out_mesh,
			  std::vector<Number>&  values,
			  std::vector<std::string>& names) const

{
  ...
  //  const unsigned int n_comp = this->n_val_per_node();
  const unsigned int n_comp = this->n_val_per_elem();


  // unsigned int MeshBase::n_active_elem  	(   	  	 )
  // Mesh::element_iterator Mesh::active_elements_begin  	(   	  	 )   	 [virtual]
...
//     values.reserve(n_comp*out_mesh.n_nodes());
  ...
  // reserve memory for the element data
    //   values.reserve(n_comp*out_mesh.n_elem());

    //***** only active elements !!!
    values.reserve(n_comp*out_mesh.n_active_elem());
...
//     const MeshBase::const_node_iterator nodes_end = out_mesh.nodes_end();
 ...
 // iterate over the mesh's elements
//     MeshBase::const_element_iterator elem_it = out_mesh.elements_begin();
//     const MeshBase::const_element_iterator elem_end = out_mesh.elements_end();

 //   Mesh::element_iterator elem_it = out_mesh.active_elements_begin();
//    Mesh::element_iterator elem_end = out_mesh.active_elements_end();

	 MeshBase::const_element_iterator     elem_it  = out_mesh.active_elements_begin();
    const MeshBase::const_element_iterator elem_end = out_mesh.active_elements_end(); 
    ...
    // when there is no nodal data.
  //   for (; nodes_it != nodes_end; ++nodes_it)
    // elem ->top_parent() gives the element's top-most (i.e. level-0) parent.
	...
	for (unsigned int c= 0; c<n_comp; c++)
	  ...
	  //	  values.push_back(this->operator()(elem, c));

	  values.push_back(this->operator()( (elem ->top_parent() ), c));

	// for  mesh refinement !!!!
    // elem ->top_parent() gives the element's top-most (i.e. level-0) parent.
	// (Returns THIS if this is a level-0 element)
...
// *********************************************
//  END  NEW :   translate  for  elements  data
//  *****************************************
...
// void MeshData_elements::close_foreign_id_maps ()
// {
//   if (_active)
//     {
...
// void MeshData_elements::read (const std::string& name)
// {
//   START_LOG("read()", "MeshData_elements");
...
//       std::cerr << "WARNING: MeshData_elements in compatibility mode, node and element ids" << std::endl
...
//       }    
//   }
//   STOP_LOG("read()", "MeshData_elements");
// }
...
// void MeshData_elements::write (const std::string& name)
// {
//   START_LOG("write()", "MeshData_elements");
...
//       }    
//   }
//   STOP_LOG("write()", "MeshData_elements");
// }




// std::string MeshData_elements::get_info() const
// {
//   std::ostringstream out;

//   if (this->active() || this->compatibility_mode())
//     {
//       out << " MeshData_elements Information:\n";
//       if (this->active())
...
//     }
//   else
//       out << " MeshData_elements neither active nor in compatibility mode.\n";
...
// void MeshData_elements::print_info(std::ostream& os) const
// {
//   os << this->get_info()
//      << std::endl;
// }
...
// std::ostream& operator << (std::ostream& os, const MeshData_elements& m)
// {
//   m.print_info(os);
//   return os;
// }
...
// const Node* MeshData_elements::foreign_id_to_node (const unsigned int fid) const
// {
//   if (_active)
//     {
...
// unsigned int MeshData_elements::node_to_foreign_id (const Node* n) const
// {
//   assert (n != NULL);
...
// const Elem* MeshData_elements::foreign_id_to_elem (const unsigned int fid) const
// {
//   if (_active)
//     {
...
// unsigned int MeshData_elements::elem_to_foreign_id (const Elem* e) const
// {
//   assert (e != NULL);
...
// void MeshData_elements::insert_node_data (std::map<const Node*,
...
// void MeshData_elements::insert_elem_data (std::map<const Elem*,
...
// unsigned int MeshData_elements::n_val_per_node () const
// {
//   assert (this->_active || this->_compatibility_mode);
...
// unsigned int MeshData_elements::n_node_data () const
// {
//   assert (this->_active || this->_compatibility_mode);
//   assert (this->_node_data_closed);
...
// unsigned int MeshData_elements::n_val_per_elem () const
// {
//   assert (this->_active || this->_compatibility_mode);
...
// unsigned int MeshData_elements::n_elem_data () const
// {
//   assert (this->_active || this->_compatibility_mode);
//   assert (this->_elem_data_closed);
...
// void MeshData_elements::assign (const MeshData_elements& omd)
// {
...
//   // the libMesh id's.  But only when the other MeshData_elements
//   // has element ids.
