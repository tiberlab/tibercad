...
void  WzLatticeThermalConductivity::read_database(void)
{
  ...
  double K11 = data("therm_lat_cond_x", 0.0);
  double K33 = data("therm_lat_cond_z", 0.0);
...
void WzLatticeThermalConductivity::do_init(void)
{
  ...
  double K11 = options.get_option("therm_lat_cond_x", _conductivity(1,1) );
  double K33 = options.get_option("therm_lat_cond_z", _conductivity(3,3) ); 
