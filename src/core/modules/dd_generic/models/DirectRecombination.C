    ...
    double& recomb_h)
{
  ...
  double n  = dd.get_q_density(get_carriers().at(0));
  double p  = dd.get_q_density(get_carriers().at(1));
  double n0 = dd.get_equilibrium_q_density(get_carriers().at(0));
  double p0 = dd.get_equilibrium_q_density(get_carriers().at(1));
  //double ni = dd.get_intrinsic_density();
  //double gn = dd.get_electron_gamma();
  //double gp = dd.get_hole_gamma();

  recomb_e = recomb_h = C_ * (n * p - n0 * p0);
}
