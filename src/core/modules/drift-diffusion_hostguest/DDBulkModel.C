...
DDBulkModel::do_init(void)
{
  ...
  if (it_bgc != end_bgc)
  {
    double sigma = 0.5 * (it_bgc->second).get_option("sigma", 1e-3 * Constants::e);
    double sigma_e = (it_bgc->second).get_option("sigma_e", sigma);
    double sigma_h = (it_bgc->second).get_option("sigma_h", sigma);

    
    _background_conductivity_e += sigma_e / Constants::e;
    _background_conductivity_h += sigma_h / Constants::e;
  }
