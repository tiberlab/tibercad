...
ExcitonTransport::do_init(void)
{
  ...
  system.add_variable("X", libMeshEnums::FIRST);
...
ExcitonTransport::build_local_scaling(void)
{
  ...
  const unsigned int var = system->variable_number("X");
  ...
  get_scaling().set_density_scaling(mean_density);
}
    ...
    SparseMatrix<Number>* jacobian)
{
  ...
  const unsigned int ex_var = system.variable_number("X");
  ...
  for ( ; el != end_el ; ++el)
  {
    ...
    for (unsigned int qp = 0; qp < qrule.n_points(); qp++)
    {
      ...
      double diffusion_coeff = em->get_mobility();
      ...
      for (unsigned int i = 0; i < n_dofs; i++)
      {
        for (unsigned int j = 0; j < n_dofs; j++)
        {
          double laplace =
            ...
            Ke(i,j) += diffusion_coeff * laplace;
        }
      }
      ...
      if (jacobian != NULL)
      {
        double dRx = C0 / R0 * em->get_net_recombination_rate_derivative();
...
void ExcitonTransport::get_solution_secure(const Elem* elem, std::map<ID, std::vector<double> >& solutions, const std::vector<Point>& points) {
    ...
    for (unsigned int n = 0; n < np; n++) {
      ...
      for (unsigned int i = 0; i < n_dofs; i++) {
        x  += phi[i][n] * ddsol(dof_indices_u[i]);
      ...
      double Dx = excitonmodel->get_mobility();
     ...
     // }
      ...
      if (solutions.count(J)) {
        solutions[J][3*n] = -Dx * grad_x(0);
        solutions[J][3*n + 1] = -Dx * grad_x(1);
        solutions[J][3*n + 2] = -Dx * grad_x(2);
      }
