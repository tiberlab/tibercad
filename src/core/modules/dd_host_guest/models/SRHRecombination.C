// $Id: SRHRecombination.C 4145 2015-10-02 11:53:20Z maufder $
    ...
    double& recomb_h)
{
  ...
  if ((n == 0.0) || (p == 0.0))
  {
    // this is for sanity only, may happen during Newton iterations
    recomb_e = 0.0;
  }
  else if ((f_e < 1e-6) && (f_h > (1.0 - 1e-6)))
  {
    ...
    // NOTE: gc and gv can be 0 <= gc, gv <= inf
    ...
    double fac = 1 / a + 1 / b - 1;  // = c / ab = f_e - f_h
    ...
    // this new formulas should never divide by 0, and never have inf/inf:
    double denom = tau_p * n / b + tau_n * p / a + tau_n * tau_p * (_gen_VT + _gen_TC) / (a*b);
    double nom = n * p * fac - (tau_p * n * (1 - 1/a) * _gen_VT / b +
        tau_n * p * (1 - 1/b) * _gen_TC / a + tau_n * tau_p * _gen_VT * _gen_TC / (a*b));

    recomb_e = nom / denom;
  }
  else
  {
    // use also that 1/a = f_e, 1/b = 1 - f_h
    double denom = tau_p * n * (1 - f_h) + tau_n * p * f_e + tau_n * tau_p * (_gen_VT + _gen_TC) * f_e * (1 - f_h);
    double nom = n * p * (f_e - f_h) - (tau_p * n * (1 - f_e) * (1 - f_h) * _gen_VT +
        tau_n * p * f_h * f_e * _gen_TC + tau_n * tau_p * _gen_VT * _gen_TC * f_e * (1 - f_h));
    ...
    std::vector<double>& recomb_e, std::vector<double>& recomb_h)
{
  ...
  */
  ...
  double dRedn = (p * (f_e - f_h) - tau_p * (1 - f_h) * (1 - f_e) * _gen_VT - r_e * tau_p * (1 - f_h)) / denom;
  double dRedp = (n * (f_e - f_h) - tau_n * f_e * f_h * _gen_TC - r_e * tau_n * f_e) / denom;
