...
DriftDiffusion::calculate_weights(void)
{
  ...
  for ( ; it != end; ++it)
  {
    ...
    for (unsigned int i = 0; i < elem->n_nodes(); i++)
    {
      ...
      sc->set_potentials(phi0 * u, phi0 * en, phi0 * ep, phi0 * en, phi0 * ep);
...
DriftDiffusion::rebuild_equation_system(void)
{
  ...
  system.add_variable("fermi_eG", libMeshEnums::FIRST);
  system.add_variable("fermi_hG", libMeshEnums::FIRST);
...
DriftDiffusion::do_setup_solution_variables(void)
{
  ...
  declare_solution(eQFermiG, REAL, NODES, "eV");
  declare_solution(hQFermiG, REAL, NODES, "eV");
  ...
  declare_solution(EcG, REAL, NODES, "eV");
  declare_solution(EvG, REAL, NODES, "eV");
  ...
  declare_solution(eDensityG, REAL, NODES, "cm^-3");
  declare_solution(hDensityG, REAL, NODES, "cm^-3");
  if (plot_solution("Density"))
  {
    add_plot_variable(eDensity);
    add_plot_variable(hDensity);
    add_plot_variable(eDensityG);
    add_plot_variable(hDensityG);
  }
  ...
  declare_solution(eCurrentDensityG, VECTOR, CELL, "A/cm^2");
  declare_solution(hCurrentDensityG, VECTOR, CELL, "A/cm^2");
    ...
    const std::vector<Point>& points)
{
  ...
  unsigned int enG_var = system->variable_number("fermi_eG");
  unsigned int epG_var = system->variable_number("fermi_hG");
  if (_useparticle == 'e')
    ep_var = en_var;
  ...
  vector<unsigned int> dof_indices_enG;
  vector<unsigned int> dof_indices_epG;
  ...
  dof_map.dof_indices(elem, dof_indices_enG, enG_var);
  dof_map.dof_indices(elem, dof_indices_epG, epG_var);
  ...
  for (unsigned int n = 0; n < np; n++)
  {
    ...
    RealGradient grad_en_locG(0);
    RealGradient grad_ep_locG(0);
    ...
    for (unsigned int i = 0; i < n_dofs; i++)
    {
      ...
      enG += phi[i][n] * solution(dof_indices_enG[i]);
      epG += phi[i][n] * solution(dof_indices_epG[i]);
      ...
      grad_en_locG += dphi[i][n] * solution(dof_indices_enG[i]);
      grad_ep_locG += dphi[i][n] * solution(dof_indices_epG[i]);
    ...
    sc->set_potentials(u, en, ep, enG, epG);
    sc->set_old_potentials(phi0 * oldu, phi0 * olden, phi0 * oldep);
    ...
    double edensG = (sc->is_dielectric() ? 0.0 : sc->get_electron_density_guest());
    double hdensG = (sc->is_dielectric() ? 0.0 : sc->get_hole_density_guest());
    ...
    double sigma_eG = Constants::e * sc->get_electron_conductivity_guest();
    double sigma_hG = Constants::e * sc->get_hole_conductivity_guest();
    ...
    jnG -= sigma_eG * grad_en_locG;
    jpG -= sigma_hG * grad_ep_locG;
    ...
    if (values.count(eQFermiG))
      values[eQFermiG][n] = -enG;

    if (values.count(hQFermiG))
      values[hQFermiG][n] = -epG;
    ...
    if (values.count(EcG))
      values[EcG][n] = sc->get_conduction_band_guest().get_band_edge() - u;

    if (values.count(EvG))
      values[EvG][n] = sc->get_valence_band_guest().get_band_edge() - u;
    ...
    if (values.count(eDensityG))
      values[eDensityG][n] = edensG;

    if (values.count(hDensityG))
      values[hDensityG][n] = hdensG;
  ...
  if (values.count(eCurrentDensityG))
  {
    values[eCurrentDensityG][0] = jnG(0) / np;
    values[eCurrentDensityG][1] = jnG(1) / np;
    values[eCurrentDensityG][2] = jnG(2) / np;
  }

  if (values.count(hCurrentDensityG))
  {
    values[hCurrentDensityG][0] = jpG(0) / np;
    values[hCurrentDensityG][1] = jpG(1) / np;
    values[hCurrentDensityG][2] = jpG(2) / np;
  }
    ...
    for (unsigned int s = 0; s < elem->n_sides(); s++)
    {
      ...
      if ((sm != NULL)  && !sm->has_current())
      {
        ...
        for (unsigned int qp = 0; qp < qface->n_points(); qp++)
        {
          ...
          sm->set_potentials(phi0 * u, phi0 * en, phi0 * ep, phi0 * en, phi0 * ep);
...
DriftDiffusion::build_local_scaling(void)
{
  ...
  DenseSubVector<Number>
    ...
    scalenG(local_scaling),
    scalepG(local_scaling);
  ...
  for ( ; it != end; ++it)
  {
    ...
    scalenG.reposition(3 * n_dofs, n_dofs);
    scalepG.reposition(4 * n_dofs, n_dofs);
    ...
    for (unsigned int qp = 0; qp < qrule->n_points(); qp++)
    {
      ...
      for (unsigned int i = 0; i < n_dofs; i++)
      {
        ...
        en += phi[i][qp] * solution(dof_indices_en[i]);
        ep += phi[i][qp] * solution(dof_indices_ep[i]);
        enG += phi[i][qp] * solution(dof_indices_enG[i]);
        epG += phi[i][qp] * solution(dof_indices_epG[i]);
        e_field += dphi[i][qp] * solution(dof_indices_u[i]);
      }
      ...
      sc->set_potentials(phi0 * u, phi0 * en, phi0 * ep, phi0 * enG, phi0 * epG);
      ...
      double sigma_eG = JxW[qp] * sc->get_electron_conductivity_guest() / mu0 / C0_e;
      double sigma_hG = JxW[qp] * sc->get_hole_conductivity_guest() / mu0 / C0_h;
      ...
      */
      ...
      double drho = -JxW[qp] * (drhovec[0] + drhovec[1] + drhovec[2] + drhovec[3]) * phi0 / C0;
      if (sc->is_dielectric())
        drho = 0.0;
    ...
    SparseMatrix<Number>* jacobian)
{
  ...
  const unsigned int en_var = system.variable_number("fermi_e");
  const unsigned int ep_var = system.variable_number("fermi_h");
  const unsigned int enG_var = system.variable_number("fermi_eG");
  const unsigned int epG_var = system.variable_number("fermi_hG");
  ...
  DenseSubMatrix<Number>
    Kuu(Ke), Kun(Ke), Kup(Ke), KunG(Ke), KupG(Ke),
    Knu(Ke), Knn(Ke), Knp(Ke), KnnG(Ke), KnpG(Ke),
    Kpu(Ke), Kpn(Ke), Kpp(Ke), KpnG(Ke), KppG(Ke),
    KnGu(Ke), KnGn(Ke), KnGp(Ke), KnGnG(Ke), KnGpG(Ke),
    KpGu(Ke), KpGn(Ke), KpGp(Ke), KpGnG(Ke), KpGpG(Ke);
  ...
  for ( ; el != end_el ; ++el)
  {
    ...
    KunG.reposition(0, 3 * n_dofs, n_dofs, n_dofs);
    KupG.reposition(0, 4 * n_dofs, n_dofs, n_dofs);
    //
    ...
    KnnG.reposition(n_dofs, 3 * n_dofs, n_dofs, n_dofs);
    KnpG.reposition(n_dofs, 4 * n_dofs, n_dofs, n_dofs);
    //
    ...
    KpnG.reposition(2 * n_dofs, 3 * n_dofs, n_dofs, n_dofs);
    KppG.reposition(2 * n_dofs, 4 * n_dofs, n_dofs, n_dofs);
    //
    KnGu.reposition(3 * n_dofs, 0, n_dofs, n_dofs);
    KnGn.reposition(3 * n_dofs, n_dofs, n_dofs, n_dofs);
    KnGp.reposition(3 * n_dofs, 2 * n_dofs, n_dofs, n_dofs);
    KnGnG.reposition(3 * n_dofs, 3 * n_dofs, n_dofs, n_dofs);
    KnGpG.reposition(3 * n_dofs, 4 * n_dofs, n_dofs, n_dofs);
    //
    KpGu.reposition(4 * n_dofs, 0, n_dofs, n_dofs);
    KpGn.reposition(4 * n_dofs, n_dofs, n_dofs, n_dofs);
    KpGp.reposition(4 * n_dofs, 2 * n_dofs, n_dofs, n_dofs);
    KpGnG.reposition(4 * n_dofs, 3 * n_dofs, n_dofs, n_dofs);
    KpGpG.reposition(4 * n_dofs, 4 * n_dofs, n_dofs, n_dofs);
    //
    Fu.reposition(0, n_dofs);
    Fn.reposition(n_dofs, n_dofs);
    Fp.reposition(2 * n_dofs, n_dofs);
    FnG.reposition(3 * n_dofs, n_dofs);
    FpG.reposition(4 * n_dofs, n_dofs);
    //
    ...
    if (_useparticle == 'e')
      ...
      Xp.reposition(2 * n_dofs, n_dofs);
    XnG.reposition(3 * n_dofs, n_dofs);
    XpG.reposition(4 * n_dofs, n_dofs);

    //
    oldXu.reposition(0, n_dofs);
    oldXn.reposition(n_dofs, n_dofs);
    oldXp.reposition(2 * n_dofs, n_dofs);
    oldXnG.reposition(3 * n_dofs, n_dofs);
    oldXpG.reposition(4 * n_dofs, n_dofs);
    //
    scaleu.reposition(0, n_dofs);
    scalen.reposition(n_dofs, n_dofs);
    scalep.reposition(2 * n_dofs, n_dofs);
    ...
    for (unsigned int qp = 0; qp < qrule->n_points(); qp++)
    {
      ...
      RealGradient grad_en(0);
      RealGradient grad_ep(0);
      RealGradient grad_enG(0);
      RealGradient grad_epG(0);
      ...
      for (unsigned int i = 0; i < n_dofs; i++)
      {
        ...
        grad_enG += dphi[i][qp] * XnG(i);
        grad_epG += dphi[i][qp] * XpG(i);
      ...
      long double nG = sc->get_electron_density_guest();
      long double pG = sc->get_hole_density_guest();
      ...
      long double RnHG = sc->get_net_electron_recombination_rateHG();
      long double RpHG = sc->get_net_hole_recombination_rateHG();
      ...
      double sigma_eG = sc->get_electron_conductivity_guest() / (mu0 * C0_e);
      double sigma_hG = sc->get_hole_conductivity_guest() / (mu0 * C0_h);
	  ...
	  if (coupling & POISSON)
          ...
          if (coupling & ECURRENT)
          {
            ...
            KnGnG(i,j) += sigma_eG * laplace / scalenG(i);
          }
          ...
          if (coupling & HCURRENT)
          {
            ...
            KpGpG(i,j) += sigma_hG * laplace / scalepG(i);
          }
        }
      ...
      if (jacobian != NULL)
      {
        ...
        double dn_dphiG = sc->get_electron_density_derivative_guest();
        double dp_dphiG = sc->get_hole_density_derivative_guest();
        ...
        drho[3] *= phi0 / C0;
        drho[4] *= phi0 / C0;
        drho[0] = -(drho[1] + drho[2] + drho[3] + drho[4]);
        ...
        long double dRn_dn_HG =
          sc->get_net_electron_recombination_rate_derivativesHG()[0];
        long double dRn_dp_HG =
          sc->get_net_electron_recombination_rate_derivativesHG()[1];
        long double dRn_dEfn_HG =
          sc->get_net_electron_recombination_rate_derivativesHG()[2];
        long double dRn_dEfp_HG =
          sc->get_net_electron_recombination_rate_derivativesHG()[3];

        long double dRnHG[3];
        long double dRpHG[3];
        dRnHG[1] = (dRn_dEfn_HG - dRn_dn_HG * dn_dphi) * phi0 / R0_e;
        dRnHG[2] = (dRn_dEfp_HG - dRn_dp_HG * dn_dphiG) * phi0 / R0_e;
        dRnHG[0] = -(dRnHG[1] + dRnHG[2]);
        dRpHG[1] = -dRnHG[1];
        dRpHG[2] = -dRnHG[2];
        dRpHG[0] = -dRnHG[0];
        //cerr << dRnHG[0] << " " << dRnHG[1] << " " << dRnHG[2] << "\n";


        // Host-Guest hl

        long double dRp_dn_HG =
          sc->get_net_hole_recombination_rate_derivativesHG()[0];
        long double dRp_dp_HG =
          sc->get_net_hole_recombination_rate_derivativesHG()[1];
        long double dRp_dEfn_HG =
          sc->get_net_hole_recombination_rate_derivativesHG()[2];
        long double dRp_dEfp_HG =
          sc->get_net_hole_recombination_rate_derivativesHG()[3];

        long double dRn2HG[3];
        long double dRp2HG[3];
        dRn2HG[1] = (dRp_dEfn_HG - dRp_dn_HG * dp_dphi) * phi0 / R0_e;
        dRn2HG[2] = (dRp_dEfp_HG - dRp_dp_HG * dp_dphiG) * phi0 / R0_e;
        dRn2HG[0] = -(dRn2HG[1] + dRn2HG[2]);
        dRp2HG[1] = -dRn2HG[1];
        dRp2HG[2] = -dRn2HG[2];
        dRp2HG[0] = -dRn2HG[0];
        //cerr << dRnHG[0] << " " << dRnHG[1] << " " << dRnHG[2] << "\n";
        ...
        double dsigma_eG = J * phi0 / (mu0 * C0_e) * mueG * dn_dphiG;
        double dsigma_hG = J * phi0 / (mu0 * C0_h) * muhG * dp_dphiG;
	...
	if (dim > 1)
	  ...
	  sc->get_electron_mobility_derivative_grad_fermi(dmu_e_grad_v);
        ...
        for (unsigned int i = 0; i < n_dofs; i++)
        {
          ...
          double lap_eG = (dphi[i][qp] * grad_enG) / scalenG(i);
          double lap_hG = (dphi[i][qp] * grad_epG) / scalepG(i);
          double dsigma_e_x_lapG = dsigma_eG * lap_eG;
          double dsigma_h_x_lapG = dsigma_hG * lap_hG;
          ...
          for (unsigned int j = 0; j < n_dofs; j++)
          {
            ...
            if (_options.exact_newton)
            {
              ...
              double dsigma_e_x_phiG = dsigma_e_x_lapG * phi[j][qp];
              double dsigma_h_x_phiG = dsigma_h_x_lapG * phi[j][qp];
              ...
              if (coupling & ECURRENT)
              {
                if (coupling & POISSON)
                {
                  ...
                  KnGu(i,j) += dsigma_e_x_phiG; // + (dmu_e_u_x_phi + dmu_e_grad_u_x_dphi) * lap_e;
                }

                Knn(i,j) += (dmu_e_grad_v_x_dphi - dmu_e_u_x_phi) * lap_e - dsigma_e_x_phi;
                KnGnG(i,j) += -dsigma_e_x_phiG; // (dmu_e_grad_v_x_dphi - dmu_e_u_x_phi) * lap_e - dsigma_e_x_phi;
              }
              ...
              if (coupling & HCURRENT)
              {
                if (coupling & POISSON)
                {
                  ...
                  KpGu(i,j) += dsigma_h_x_phiG; // + (dmu_h_u_x_phi + dmu_h_grad_u_x_dphi) * lap_h;
                }

                Kpp(i,j) += (dmu_h_grad_w_x_dphi - dmu_h_u_x_phi) * lap_h - dsigma_h_x_phi;
                KpGpG(i,j) += -dsigma_h_x_phiG; //(dmu_h_grad_w_x_dphi - dmu_h_u_x_phi) * lap_h - dsigma_h_x_phi;
              }
		  ...
		  Kpu(i,j) += elem_contrib;
            ...
            if (coupling & POISSON)
            {
              ...
              if (coupling & ECURRENT)
              {
                ...
                KunG(i,j) -= drho[3] * phi_i_x_phi_j / scaleu(i);
              }
              ...
              if (coupling & HCURRENT)
              {
                ...
                KupG(i,j) -= drho[4] * phi_i_x_phi_j / scaleu(i);
              }
            }
            ...
            if (coupling & ECURRENT)
            {
              if ((coupling & POISSON) && _options.exact_newton)
              {
                ...
                Knu(i,j) -= (dRn[0] + dRnHG[0]) * phi_i_x_phi_j / scalen(i);
                KnGu(i,j) -= dRpHG[0] * phi_i_x_phi_j / scalenG(i);
              }

              //Knn(i,j) -= dRn[1] * phi_i_x_phi_j / scalen(i);
              Knn(i,j) -= (dRn[1] + dRnHG[1]) * phi_i_x_phi_j / scalen(i);
              KnnG(i,j) -= dRnHG[2] * phi_i_x_phi_j / scalen(i);

              KnGn(i,j) -= dRpHG[1] * phi_i_x_phi_j / scalenG(i);
              KnGnG(i,j) -= dRpHG[2] * phi_i_x_phi_j / scalenG(i);
            ...
            if (coupling & HCURRENT)
            {
              if ((coupling & POISSON) && _options.exact_newton)
              {
                ...
                Kpu(i,j) -= (dRn2HG[0] - dRp[0]) * phi_i_x_phi_j / scalep(i);
                KpGu(i,j) -= dRp2HG[0] * phi_i_x_phi_j / scalepG(i);
              }
              ...
              Kpp(i,j) += (dRp[2] - dRn2HG[1]) * phi_i_x_phi_j / scalep(i);
              KppG(i,j) -= dRn2HG[2] * phi_i_x_phi_j / scalep(i);

              KpGp(i,j) -= dRp2HG[1] * phi_i_x_phi_j / scalepG(i);
              KpGpG(i,j) -= dRp2HG[2] * phi_i_x_phi_j / scalepG(i);
            }
      ...
      if (residual != NULL)
      {
        ...
        long double J_x_RnHG = J * RnHG / R0_e;
        long double J_x_RpHG = J * RpHG / R0_e;
        ...
        for (unsigned int i = 0; i < n_dofs; i++)
        {
          long double net_recomb_e = (J_x_Rn + J_x_RnHG) * phi[i][qp] / scalen(i);
          long double net_recomb_h = (-J_x_Rp + J_x_RpHG) * phi[i][qp] / scalep(i);
          long double net_recomb_eHG = J_x_RnHG * phi[i][qp] / scalenG(i);
          long double net_recomb_pHG = J_x_RpHG * phi[i][qp] / scalepG(i);
  ...
  if (jacobian != NULL)
  {
    ...
    */
    ...
    if (__private_counter > 10) exit(0);
    */
  }
  else
  {
...
DriftDiffusion::write_nodal_vector(const string& filename, const NumericVector<double>& vec)
{
  ...
  for ( ; it != end; ++it)
  {
    ...
    for (unsigned int n = 0; n < elem->n_nodes(); n++)
    {
      unsigned int id = 5 * elem->node(n);
      results[id] = vec(dof_indices_u[n]);
      results[id + 1] = vec(dof_indices_en[n]);
      results[id + 2] = vec(dof_indices_ep[n]);
      results[id + 3] = vec(dof_indices_enG[n]);
      results[id + 4] = vec(dof_indices_epG[n]);
    }
