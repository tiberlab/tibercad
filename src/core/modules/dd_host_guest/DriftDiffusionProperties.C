...
DriftDiffusionProperties::PointData::PointData(void)
  : //electron_conductivity_derivatives(3, 0.0),
    ...
    electron_recombination_rateHG(0.0),
    electron_recombination_rate_derivatives(4, 0.0),
    electron_recombination_rate_derivativesHG(4, 0.0),
    hole_recombination_rate(0.0),
    hole_recombination_rate_derivatives(4, 0.0),
    hole_recombination_rate_derivativesHG(4, 0.0)
{
}
...
DriftDiffusionProperties::DriftDiffusionProperties(const ModelOptions& options)
  : PhysicalModel(options),
    ...
    _valence_band(NULL),
    _conduction_band_guest(NULL),
    _valence_band_guest(NULL),
...
DriftDiffusionProperties::prepare_submodels(void)
{
  ...
  if (get_options().has_submodel("conduction_band_guest"))
  {
    if (_conduction_band_guest != NULL)
    throw InitFailedException("Multiple definition of conduction band properties for material "
        + get_material()->get_name());

    ModelOptions opts(get_options().submodels_begin("conduction_band_guest")->second);
    opts.set_option("particle", "el");
    opts.set_option("band_type", "guest");
    get_options().add_submodel("band_properties", opts);
    get_options().delete_submodels("conduction_band_guest");
  }

  if (get_options().has_submodel("valence_band_guest"))
  {
    if (_valence_band_guest != NULL)
    throw InitFailedException("Multiple definition of valence band properties for material "
        + get_material()->get_name());

    ModelOptions opts(get_options().submodels_begin("valence_band_guest")->second);
    opts.set_option("particle", "hl");
    opts.set_option("band_type", "guest");
    get_options().add_submodel("band_properties", opts);
    get_options().delete_submodels("valence_band_guest");
  }
  ...
  for (size_t i = 0; i < bp.size(); i++)
  {
    ...
    {
      if (string("guest") == bp[i]->get_options().get_option("band_type", "host"))
        _conduction_band_guest = bp[i];
      else
...
void
DriftDiffusionProperties::set_conduction_band_guest(BandProperties* cb)
{
  _conduction_band_guest = cb;
}


void
DriftDiffusionProperties::set_valence_band_guest(BandProperties* vb)
{
  _valence_band_guest = vb;
}
...
DriftDiffusionProperties::calculate_densities(void)
{
  ...
  BandProperties& cbG = get_conduction_band_guest();
  BandProperties& vbG = get_valence_band_guest();
  cbG.set_temperature(kTe);
  vbG.set_temperature(kTh);
  ...
  el = cbG.get_density_and_derivative(_pd->fermi_eG, _pd->electric_potential);
  _pd->electron_densityG = el.first;
  _pd->electron_density_derivativeG = el.second;
  ...
  hl = vbG.get_density_and_derivative(_pd->fermi_hG, _pd->electric_potential);
  _pd->hole_densityG = hl.first;
  _pd->hole_density_derivativeG = hl.second;
...
DriftDiffusionProperties::calculate_net_recombination_rates(void)
{
  ...
  _pd->electron_recombination_rateHG = 0;
  _pd->electron_recombination_rate_derivativesHG[0] = 0;
  _pd->electron_recombination_rate_derivativesHG[1] = 0;
  _pd->electron_recombination_rate_derivativesHG[2] = 0;
  _pd->electron_recombination_rate_derivativesHG[3] = 0;
  _pd->hole_recombination_rateHG = 0;
  _pd->hole_recombination_rate_derivativesHG[0] = 0;
  _pd->hole_recombination_rate_derivativesHG[1] = 0;
  _pd->hole_recombination_rate_derivativesHG[2] = 0;
  _pd->hole_recombination_rate_derivativesHG[3] = 0;
  ...
  for ( ; it != end; ++it)
  {
    ...
    else if ((it->second)->coupling() == string("HG"))
    {
      (it->second)->get_net_recombination_rates(Re, Rh);
      (it->second)->get_net_recombination_rate_derivatives(dRe, dRh);

      _pd->electron_recombination_rateHG += Re;
      _pd->electron_recombination_rate_derivativesHG[0] += dRe[0];
      _pd->electron_recombination_rate_derivativesHG[1] += dRe[1];
      _pd->electron_recombination_rate_derivativesHG[2] += dRe[2];
      _pd->electron_recombination_rate_derivativesHG[3] += dRe[3];

      _pd->hole_recombination_rateHG += Re;
      _pd->hole_recombination_rate_derivativesHG[0] += dRe[0];
      _pd->hole_recombination_rate_derivativesHG[1] += dRe[1];
      _pd->hole_recombination_rate_derivativesHG[2] += dRe[2];
      _pd->hole_recombination_rate_derivativesHG[3] += dRe[3];
    }
    else if ((it->second)->coupling() == string("eHG"))
    {
      (it->second)->get_net_recombination_rates(Re, Rh);
      (it->second)->get_net_recombination_rate_derivatives(dRe, dRh);
    ...
    else if ((it->second)->coupling() == string("hHG"))
    {
