// $Id: DDBulkModel.C 4145 2015-10-02 11:53:20Z maufder $
...
DDBulkModel::calculate_mobilities(void)
{
  ...
  else
  {
    ...
    pd.electron_mobilityG = pd.electron_mobility / 100;
    pd.hole_mobilityG = pd.hole_mobility / 100;
  }
  ...
  pd.electron_conductivityG =
    pd.electron_mobility * pd.electron_densityG + _background_conductivity_e;
  pd.hole_conductivityG =
    pd.hole_mobility * pd.hole_densityG + _background_conductivity_h;
...
DDBulkModel::calculate_equilibrium_properties(void)
{
  ...
  */
  ...
  for (unsigned int i = 0; i < 10000; ++i)
  {
    ...
    double df_fermi[4];
    get_charge_density_derivatives(df_fermi);
    double df = -(df_fermi[0] + df_fermi[1]+ df_fermi[2] + df_fermi[3]);
